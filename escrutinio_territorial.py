...
# Regex unico para todos os territorios
nomes_ordenados = sorted(todos_nomes.keys(), key=len, reverse=True)
TERR_RE = re.compile(r'\b(' + '|'.join(re.escape(n) for n in nomes_ordenados if len(n) > 3) + r')\b', re.IGNORECASE)
