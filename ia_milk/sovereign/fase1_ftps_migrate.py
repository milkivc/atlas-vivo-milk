...
import base64, ftplib, hashlib, io, json, pathlib, secrets, time
import requests
...
('1EcjRxUSm8VAdjDIGD-9mMFTDcd6WpcCXqgeUADEJzOU','sheet','GENEALOGIA.xlsx'),
('1Pm-kIm8EJJxg5e2Gd3qnyPnJCBfiT2di','stored','LOTE023_DOSSIE.html'),
...
def consume_privatebin(url):
    ...
    d=webdriver.Chrome(options=o); pw=''
    try:
...
def ensure_dest(pw):
    f=connect(pw)
    try:
        data=secrets.token_bytes(4096); sh=hashlib.sha256(data).hexdigest(); n=f'canary-{int(time.time())}.bin'
        f.storbinary('STOR '+n,io.BytesIO(data)); h=hashlib.sha256(); c=[0]
...
def main():
    kp=pathlib.Path('/tmp/ia_milk_fase1.key'); sp=pathlib.Path('/tmp/fase1_payload.sealed.b64')
    plain=SealedBox(PrivateKey(kp.read_bytes())).decrypt(base64.urlsafe_b64decode(sp.read_text().strip().encode()))
    cfg=json.loads(plain); u=cfg['ptservidor_paste_url']; mask(u); pw=consume_privatebin(u); mask(pw); print('PRIVATEBIN_CREDENTIAL_CONSUMED_AND_MASKED')
    ensure_dest(pw); work=pathlib.Path('/tmp/ia_milk_fase1'); work.mkdir(mode=0o700,exist_ok=True)
    rec={'state':'FASE1_PRIORITY_FTPS_MIGRATION','source':'Google Drive read-only/direct export','source_read_only':True,'destination':'PTServidor private FTPS','remote_folder':REMOTE_FOLDER,'expected_items':len(ITEMS),'verified_items':0,'verified_bytes':0,'failed_items':0,'items':[],'secrets_exposed':False,'mistral_received_secrets':False}
    rp=pathlib.Path('ia-milk-fase1-ftps-receipt.json')
    for fid,kind,name in ITEMS:
        ...
        try:
            ...
            local=h.hexdigest(); r.update(local_bytes=n,local_sha256=local)
            f=connect(pw)
            try:
                f.cwd(REMOTE_FOLDER)
                ...
                f.retrbinary('RETR '+name,rx,blocksize=1024*1024)
                r.update(remote_bytes=rn[0],remote_sha256=rh.hexdigest()); r['verified']=(rn[0]==n and rh.hexdigest()==local)
    ...
    rec['complete_subset']=rec['verified_items']==len(ITEMS) and rec['failed_items']==0
    rp.write_text(json.dumps(rec,ensure_ascii=False,indent=2)); print('FASE1_MIGRATION_RESULT '+json.dumps({k:rec[k] for k in ['verified_items','failed_items','verified_bytes','complete_subset']}))
    pw='0'*len(pw); u=''; plain=b'0'*len(plain); kp.unlink(missing_ok=True); sp.unlink(missing_ok=True)
    return 0 if rec['complete_subset'] else 3
