...
def parse_file_blocks(raw):
    ...
    if not matches: raise ValueError('agent did not return file blocks')
    return {path.strip():content for path,content in matches}

def write_files(files):
    written=[]
    for path,content in files.items():
        if not path.startswith(ALLOWED_PREFIXES): raise ValueError('unauthorized path: '+path)
        if '..' in pathlib.PurePosixPath(path).parts: raise ValueError('path traversal denied')
        p=ROOT/path; p.parent.mkdir(parents=True,exist_ok=True); p.write_text(content,encoding='utf-8'); written.append(path)
    return written

by={a.get('name'):a for a in agents()}
web_name='MILK Research Decoder — Web App'
ia_name='ZecaBrito'
if web_name not in by or ia_name not in by: raise SystemExit('required Mistral agents missing')
...
for idx,(name,prompt) in enumerate(batches,1):
    try:
        raw=run_agent(by[name]['id'],prompt); files=parse_file_blocks(raw); written=write_files(files)
...
pathlib.Path('artifacts/mistral-build-receipt.json').write_text(json.dumps({'state':state,'results':results,'errors':errors,'external_publication_writes':0,'drive_writes':0},ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps({'state':state,'successful_batches':len(results),'failed_batches':len(errors)},ensure_ascii=False))
if not (ROOT/'webapp/package.json').exists() or not (ROOT/'ia-milk/models.py').exists(): raise SystemExit('core generation incomplete')
